...
This plugin is a customize RSS feed plugin. It's a forked version of `gatsby-plugin-feed`, now dubbed `gatsby-plugin-feed2`

I made this because `node-rss` doesn't support stitcher.com podcast site, because it doesn't like `[CDATA]` inside of the `<title>` and `<description>` tags on the RSS.xml

I pulled in `gatsby-plugin-feed` and modified it so it doesn't need a compilation step, and convert it to purely `nodeJS`. This meant modifying `import` statements to instead be `require`, and some fixes towards async/await issues with nodes `fs` module

On the `node-rss` package side, I imported the dependencies into `package.json` with the plugin. I modified `rss.js` to also include an optional parameter called `disable_cdata`, this is by default false. If enabled to true, if removes `[CDATA]` for `<title>` and `<description>`
